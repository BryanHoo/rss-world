...
- 继续允许最多 3 条提醒进入队列
- 最新进入的提醒显示在堆叠顶部，作为主视觉焦点；已存在提醒向下顺延
- `success` 与 `info` 继续自动消失
- 所有 tone 继续允许手动关闭，以保留现有行为
- `error` 保留更长展示时长，但不额外改动 store 默认时长策略
...
- `src/features/toast/ToastHost.tsx`
  ...
  - 继续持有 `setApiErrorNotifier / clearApiErrorNotifier` 桥接，确保 API 错误仍能进入全局提醒系统
...
- store 内部队列顺序默认不改；如需让最新提醒出现在顶部，只在 `ToastHost` 展示层处理渲染顺序
...
- 右侧为全部 tone 保留关闭按钮，但降权处理，让它仅作为补充控制
...
- `src/features/toast/ToastHost.test.tsx`
  ...
  - 断言最新提醒出现在顶部，且各 tone 仍可通过关闭按钮手动关闭
  - 保留现有消息可见与卸载清空行为验证
  - 保留 `setApiErrorNotifier / clearApiErrorNotifier` 桥接初始化与卸载清理验证
- 如需调整展示契约
